...
def load_csv_data(data_path: str) -> pd.DataFrame:
    """Load data from csv data locally.

    Args:
        path (Path): Path of the csv file.

    Returns:
        A pandas DataFrame with loaded csv data.

    """
    data = pd.read_csv(Path(data_path, "train") + "train.csv")
    return data
...
def set_seed(seed: int = 1234) -> None:
    ...
    torch.cuda.manual_all_seed(seed)  # Multi GPU
...
def dict_diff(d_a: Dict, d_b: Dict, d_a_name="a", d_b_name="b") -> Dict:
    ...
    if d_a.keys() != d_b.keys():
        raise Exception("Cannot compare there dictionaries because they have different keys.")
